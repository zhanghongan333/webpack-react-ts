...
const handleLabelLength = (label: any) => {
...
G6.registerNode('dice-er-box', {
  draw: function draw(cfg: any, group: any) {
    ...
    const radius = attrs.length > 0 ? [boxStyle.radius, boxStyle.radius, 0, 0] : boxStyle.radius
  ...
  setState(name: any, value: any, item: any) {
    ...
    if (name && name.startsWith('highlight')) {
      ...
      // 查找 label 元素，通过下标来找
      const label = shape.get('parent').get('children')[3].get('children')[anchorIndex * 2 + 1]
...
G6.registerEdge('dice-er-edge', {
  draw: function draw(cfg: any, group: any) {
    ...
    const sourceIndex = sourceNode.attrs.findIndex((e: any) => e.key === cfg.sourceAnchor)
    ...
    if (sourceIndex > sourceStartIndex - 1) {
      sourceY = itemHeight + (sourceIndex - sourceStartIndex + 0.5) * itemHeight
    }

    const targetIndex = targetNode.attrs.findIndex((e: any) => e.key === cfg.targetAnchor)
    ...
    if (targetIndex > targetStartIndex - 1) {
      targetY = (targetIndex - targetStartIndex + 0.5) * itemHeight + itemHeight
    }
